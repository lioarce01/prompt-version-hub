...
export class TestsService {
   ...
   * Create a test case
   */
  async createTestCase(promptName: string, userId: string, params: CreateTestCaseParams): Promise<TestCase> {
    // Get the prompt
    const { data: prompt, error: promptError } = await this.supabase
      .from('prompts')
      .select('*')
    ...
    if (prompt.created_by !== userId) {
      throw new Error('You do not have permission to create test cases for this prompt');
    }
   ...
   * Update a test case
   */
  async updateTestCase(caseId: number, userId: string, params: UpdateTestCaseParams): Promise<TestCase> {
    // Get the test case
   ...
   */
  async generateTestCases(promptName: string, userId: string, count: number): Promise<TestCase[]> {
    // Get the prompt
    const { data: prompt, error: promptError } = await this.supabase
      .from('prompts')
      .select('*')
    ...
    // Check permission
    if (prompt.created_by !== userId) {
      throw new Error('You do not have permission to generate test cases for this prompt');
    }

    // Call Edge Function to generate test cases
    const { data, error } = await this.supabase.functions.invoke('generate-test-cases', {
      body: {
        prompt_id: prompt.id,
   ...
   */
  async runTests(promptName: string, userId: string, params: RunTestsParams): Promise<TestRun[]> {
    // Get the prompt
    const { data: prompt, error: promptError } = await this.supabase
      .from('prompts')
      .select('*')
